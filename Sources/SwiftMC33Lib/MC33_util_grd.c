...
/*
	...
	struct.
*/
_GRD* grid_from_data_pointer(unsigned int Nx, unsigned int Ny, unsigned int Nz, double *r0, double *d, GRD_data_type* data) {
	if (!data || Nx == 0 || Ny == 0 || Nz == 0)
		return 0;
	unsigned int i, j, k;
	_GRD *Z = (_GRD*)malloc(sizeof(_GRD));
    memcpy(Z->r0, r0, sizeof r0);
    memcpy(Z->d, d, sizeof d);
	if (!Z) return 0;
	Z->internal_data = 0;
